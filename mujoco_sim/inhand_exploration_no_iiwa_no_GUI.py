...
step = 0
data = np.zeros([32])
t_step_flag = time.time()


def pose_joints_cb(state):
    global data, contacts_one_step, flag_reached_goal, step, t_step_flag
...
suffix = 198  # vecT_mode = GP_projection
objectNames = ['finger_tip', 'apc_red_bowl', 'apc_2x2', 'bunny']
batchExperiments_all = ['00', '01', '10', '11']
# batchExperiments_all = ['00', '01']
# objectNames = [ 'apc_red_bowl']


for batch in batchExperiments_all:
    suffix = int('2' + batch + '00')
    for obj_name in objectNames:
        if batch =='00' and obj_name == 'finger_tip':
            continue
        for trial in range(10):
            print('###########   Start trial', trial, ' batch=', batch, 'obj_name=', obj_name)
            contacts_one_step = [[], [], [], []]
            flag_reached_goal = 0
            step = 0
            data = np.zeros([32])
            ...
            while 1:
                ...
                if first_pose:
                    ...
                    delta_t = time.time() - t_step_flag
                    if np.linalg.norm(r.dx[:3]) < 0.003 * 3 and np.linalg.norm(
                            xd - x) < 0.002:  # vel threshold +  position error
                        if flag_reached_goal == 0:
                            flag_reached_goal = 1
                            print(batch, '-',  obj_name, ': Trial', trial, 'step', step, ". Goal reached, which costs ", delta_t, '(s)')

                    if delta_t > 20:
                        print('Time out for this step', step)
                        t_step_flag = time.time()
                        flag_reached_goal = 1
                    # else:
                    #     flag_reached_goal = 0
                ...
                for i, tip in enumerate(tip_site):
                    ...
                    if GUI:
                        r.view_vector(current_contact_pos[i, :], 0.1 * tips_normal_data[i, :],
                                      rgba=np.array([0.4, 0.8, 1, 0.7]))
                ...
                for i, pos in enumerate(r.exploration_contact):
                    ...
                    if len(contact_points[i]) != 0 and GUI:
            ...
            while os.path.isfile(save_path + '.npy'):
                save_path = save_path[:-len(str(suffix))] + str(int(save_path[-len(str(suffix)):]) + 1)
