...
function [H] = mySGDHMatrix(v, symbolic)

    if nargin < 2
        symbolic = false;
    end
    if nargin < 1
        v = [sym('a'),sym('A'),sym('t'),sym('D')];
    end
    ...
    if symbolic % v should be a symbolic vector
        H = sym([ cos(theta)  -sin(theta)*cos(alpha)   sin(theta)*sin(alpha)   A*cos(theta);
